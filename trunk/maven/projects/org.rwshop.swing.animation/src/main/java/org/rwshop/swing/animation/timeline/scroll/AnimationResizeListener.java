 ...
 */
public class AnimationResizeListener extends EditorListener{
     ...
     * @param panel
     */
    public AnimationResizeListener(AnimationTimelinePanel panel){
        myPanel = panel;
    }
     ...
     */
    public void setSize(){
        int animWidth = myPanel.getMinWidth();
        int min = myPanel.getMinimumSize().width;
        if(animWidth > min){
            int pref = myPanel.getPreferredSize().width;
            if(animWidth > pref){
                myPanel.setPreferredSize(new Dimension(animWidth, 0));
            }
            myPanel.setMinimumSize(new Dimension(animWidth, 0));
            myPanel.revalidate();
        }
    }
