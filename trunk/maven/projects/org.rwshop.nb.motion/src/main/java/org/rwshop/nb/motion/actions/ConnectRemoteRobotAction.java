...
public final class ConnectRemoteRobotAction implements ActionListener {
	private static final Logger theLogger = LoggerFactory.getLogger(ConnectRemoteRobotAction.class);
	...
	public void actionPerformed(ActionEvent e) {
		Thread connectThread = new Thread(new Runnable() {
			...
			public void run() {
				...
				theLogger.info("Connecting to robot {}", robotId);
				...
				try {
					...
					theLogger.error("Can''t connect to robot at {}: {}",
							ip, ex.getMessage());
					ex.printStackTrace();
	...
	private Set<ServiceRegistration> connectMotion(
		...
		Set<ServiceRegistration> regs = new HashSet<>(4);
		...
		try {
			...
			theLogger.error("Unable to start connection: {}", connectionId, ex);
			return null;
		}
	...
	private static Set<OSGiComponent> startRemoteRobotClientServices(
		...
		Set<OSGiComponent> comps = new HashSet<>(4);
		BytesMessageBlockingReceiverLifecycle<RobotResponse> respRecLifecycle =
				new BytesMessageBlockingReceiverLifecycle<>(
